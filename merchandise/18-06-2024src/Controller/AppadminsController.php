...
GROUP BY `in_products`.`prod_id`')->fetchAll('assoc');
    ...
    public function browseProducts($payment_id)
    {
        ...
        if (!empty($prev_products)) {
            /* $product_list */
            $product_list1 = $this->InProducts->find('all')->contain(['brand', 'pop'])->order(['InProducts.id' => 'DESC'])->where(['InProducts.profile_type' => $gender, /* 'InProducts.prod_id NOT IN' => $prev_products, 'InProducts.quantity >' => 0, */ 'InProducts.match_status' => 2, 'InProducts.allocate_to_user_id IS' => NULL, 'InProducts.allocate_to_kid_id IS' => NULL])->group('prod_id');
        } else {
            /* $product_list */
            $product_list1 = $this->InProducts->find('all')->contain(['brand', 'pop'])->order(['InProducts.id' => 'DESC'])->where(['InProducts.profile_type' => $gender, /* 'InProducts.quantity >' => 0, */ 'InProducts.match_status' => 2, 'InProducts.allocate_to_user_id IS' => NULL, 'InProducts.allocate_to_kid_id IS' => NULL])->group('prod_id');
        }
        if (!empty($_GET['search_for']) && !empty($_GET['search_data'])) {
            if ($_GET['search_for'] == "product_name1") {
                $get_data_value = trim($_GET['search_data']);
