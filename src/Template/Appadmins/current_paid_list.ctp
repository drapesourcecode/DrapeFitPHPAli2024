                                    ...
                                    <td colspan="10">
                                        ...
                                        <div class="col-sm-6">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <!--<input type="hidden" name="work_flow" value="<?= (!empty($_GET['work_flow'])) ? $_GET['work_flow'] : ""; ?>" required>-->
                                                <button type="submit" class="btn btn-sm btn-info">Search</button>
                                                <a href="<?= HTTP_ROOT; ?>appadmins/current_paid_list" class="btn btn-sm btn-primary">See All</a>
                                            </div>
                                            <?= $this->Form->end() ?>
                                            ...
                                            <select class="form-control" name="work_flow">
                                                
                                                    <option value="" selected >Select field</option>
                                                <option <?= (!empty($_GET['work_flow']) && ($_GET['work_flow'] == Configure::read('Onboard_in_Progress') ) ) ? "selected" : ""; ?> value="<?=Configure::read('Onboard_in_Progress');?>"><?=Configure::read(Configure::read('Onboard_in_Progress'));?></option>
                                                <option <?= (!empty($_GET['work_flow']) && ($_GET['work_flow'] == Configure::read('Stylist_Queue') ) ) ? "selected" : ""; ?> value="<?=Configure::read('Stylist_Queue');?>"><?=Configure::read(Configure::read('Stylist_Queue'));?></option>
                                                <option <?= (!empty($_GET['work_flow']) && ($_GET['work_flow'] == Configure::read('Inv_Queue') ) ) ? "selected" : ""; ?> value="<?=Configure::read('Inv_Queue');?>"><?=Configure::read(Configure::read('Inv_Queue'));?></option>
                                                <option <?= (!empty($_GET['work_flow']) && ($_GET['work_flow'] == Configure::read('QA_Queue') ) ) ? "selected" : ""; ?> value="<?=Configure::read('QA_Queue');?>"><?=Configure::read(Configure::read('QA_Queue'));?></option>
                                                <!--<option <?= (!empty($_GET['work_flow']) && ($_GET['work_flow'] == Configure::read('Support_Tab_Ready_to_Ship_Queue') ) ) ? "selected" : ""; ?> value="<?=Configure::read('Support_Tab_Ready_to_Ship_Queue');?>"><?=Configure::read(Configure::read('Support_Tab_Ready_to_Ship_Queue'));?></option>-->
                                            </select>
                                            <?php } ?>
                                            </div>
                                            <div class="form-group">
                                                <label for="selected_date">Select Date</label>
                                                <input type="hidden" id="search_for" name="search_for" class="form-control" value="selected_date">
                                                <input style="height: 35px; width: 250px;font-weight: bold;" type="date" name="selected_date" autocomplete="off" placeholder="search" value="<?= (!empty($_GET['selected_date'])) ? $_GET['selected_date'] : ""; ?>" >
                                ...
                                ?>
                                    ...
                                    <tr>
                                        ...
                                        <?php if (!in_array($type, [3, 7, 8, 9])) { ?>
                                            ...
                                            <td>
                                                <div>
                                                    Stylist: <?php echo $this->Custom->previousStyleistName(@$pages->user_id, @$pages->id, @$pages->count); ?> </div>
                                                <div>
                                                    Inventory: <?php echo $this->Custom->previousInvName(@$pages->user_id, @$pages->id, @$pages->count); ?> </div>
                                                <div>
                                                    Qa: <?php echo $this->Custom->previousQaName(@$pages->user_id, @$pages->id, @$pages->count); ?> </div>
                                                <div>
                                                    Support: <?php echo $this->Custom->previousSupportName(@$pages->user_id, @$pages->id, @$pages->count); ?> </div>
                                            </td>
                                            ...
                                            <td>
                                                <div class="btn-boxes">
                                                    ...
                                                    ?>
                                                        ...
                                                        <a href="<?= HTTP_ROOT; ?>appadmins/previous_order_list/<?= $pages->user_id; ?>" class="btn btn-info">Previous Order</a>
                                                        <button type="button" onclick="openCmt(<?= $pages->id; ?>)" class="btn btn-primary">Comments</button>
                                                        <?php if ($type == 1) { ?>
                                                            <a href="<?= HTTP_ROOT; ?>appadmins/updateWorkFlowStatusToNotshipnow/<?= $pages->id; ?>" class="btn btn-info">Can't ship now</a>
                                                            <?php if ($pages->work_flow_status == Configure::read('Onboard_in_Progress')) { ?>
                                                                <a href="<?= HTTP_ROOT; ?>appadmins/updateWorkFlowStatusToStylist/<?= $pages->id; ?>" class="btn btn-info">Assign to stylist</a>
                                                            <?php } ?>
                                                        <?php } ?>
                                                    <?php
                                            ...
                                            <td style="width: 92px;">
                                                <div class="btn-boxes">
                                                    <?php if (@$pages->user->kids_detail->id && @$pages->profile_type == 3) { ?>
                                                        ...
                                                        <!--                                                    <a href="<?php echo HTTP_ROOT . 'appadmins/df_matching/' . @$pages->id; ?>" data-placement="top"  class="btn btn-info" ><i class="fa fa-magic" aria-hidden="true"></i>DF Matching</a>-->
                                                        <a href="<?= HTTP_ROOT; ?>appadmins/completeUserProfileSataus/<?= $pages->id; ?>" class="btn btn-info" onclick="return confirm('Before proceed confirm all products are checkout.');" title="Move to previous work list">MPWL</a>
                                                        <a href="<?= HTTP_ROOT; ?>appadmins/previous_order_list/<?= $pages->user_id; ?>/<?= $pages->kid_id; ?>/" class="btn btn-info">Previous Order</a>
                                                        <button type="button" onclick="openCmt(<?= $pages->id; ?>)" class="btn btn-primary">Comments</button>
                                                        <?php if ($type == 1) { ?>
                                                            <a href="<?= HTTP_ROOT; ?>appadmins/updateWorkFlowStatusToNotshipnow/<?= $pages->id; ?>" class="btn btn-info">Can't ship now</a>
                                                            <?php if ($pages->work_flow_status == Configure::read('Onboard_in_Progress')) { ?>
                                        ...
                                        <?php } ?>
                                        <?php if($type == 1){ ?>   
                                        <td><?= empty($pages->work_flow_status) ? '' : Configure::read($pages->work_flow_status); ?></td>
                                        <?php } ?>
...
<script>
    ...
    $.ajax({
        type: "POST",
        url: "<?=HTTP_ROOT;?>appadmins/editComment",
