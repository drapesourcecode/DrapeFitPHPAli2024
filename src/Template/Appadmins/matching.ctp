<div class="content-wrapper">
    ...
    <section class="content">
        <div class="row">
            ...
            <div  class="col-sm-6">
                <a href="<?= HTTP_ROOT.'appadmins/allallocation/'.$id.'/'.$getData->user_id.'/'.$getData->kid_id; ?>" class="btn btn-sm btn-primary">All Allocation Prediction</a>
...
//                                foreach ($getProducts as $key => $prodDetls):
                                foreach ($getProducts as $ap_key => $ap_li):
                                    // dd($ap_li['id']);
                                    $key = $ap_li['id'];
                                    // $key=$ap_li->id;  // this is when used with all_products
                                    ?>
                                    <tr>
                                        <td> 
<!--                                            <span style="display:none;"><?php echo 1000-count($getProducts[$ap_li->id]);;/*count($getProducts[$ap_li->id]) * 10*/ ?></span>
                                        ...
                                        <?php 
                                            foreach ($all_products as $kk => $value) {
                                                if ($key == $value->id) {
                                                    echo " color matches";
                                                }
                                            }
                                            // if ($all_products[$ap_li->id] ?? false) {
                                            //     echo 'matches';
                                            // }
                                        ?>
                                        </td>
